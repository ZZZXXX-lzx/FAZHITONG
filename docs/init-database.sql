-- ============================================================
-- 法智通法律服务平台 - 数据库一键初始化脚本
-- 执行顺序: 1.建库建表 → 2.扩展功能表 → 3.初始数据
-- 使用方法: mysql -u root -p < init-database.sql
-- ============================================================

-- ===== Part 1: 基础表结构 =====
CREATE DATABASE IF NOT EXISTS fazhitong DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE fazhitong;
...
(2, '什么是诉讼时效？', '了解诉讼时效的基本概念和法律规定', '诉讼时效是指权利人在法定期间内不行使权利，就丧失了请求人民法院保护其民事权利的法律制度。我国《民法典》规定，一般诉讼时效为三年，自权利人知道或者应当知道权利受到损害以及义务人之日起计算。', '诉讼时效,民法典,民事权利', '法智通'),
(2, '如何写一份有效的借条？', '借条的法律要素和注意事项', '借条是表明债权债务关系的书面凭证。一份有效的借条应当包含：借款人和出借人的身份信息、借款金额（大小写）、借款用途、借款期限、利率约定（不得超过法律规定的上限）、借款日期、借款人签字捺印。', '借条,借贷,债权债务', '法智通'),
(3, '合同违约后的法律救济', '合同违约的赔偿方式和救济途径', '当一方违约时，守约方可以要求继续履行、采取补救措施、赔偿损失等。赔偿额应当相当于因违约所造成的损失，包括合同履行后可以获得的利益。', '合同违约,赔偿,救济', '法智通'),
(4, '离婚财产如何分割？', '离婚时夫妻共同财产的分割原则', '离婚时，夫妻共同财产由双方协议处理；协议不成时，由人民法院根据财产的具体情况，按照照顾子女、女方和无过错方权益的原则判决。', '离婚,财产分割,婚姻法', '法智通'),
(5, '用人单位拖欠工资怎么办？', '劳动者追讨工资的法律途径', '用人单位拖欠工资的，劳动者可以：1.向劳动监察部门投诉；2.申请劳动仲裁；3.对仲裁结果不服的可以向法院起诉；4.申请支付令。', '拖欠工资,劳动仲裁,维权', '法智通'),
(6, '交通事故赔偿标准', '交通事故人身损害赔偿的计算标准', '交通事故赔偿项目包括：医疗费、误工费、护理费、交通费、住宿费、住院伙食补助费、营养费、残疾赔偿金、残疾辅助器具费、被扶养人生活费、精神损害抚慰金等。', '交通事故,赔偿,人身损害', '法智通'),
(7, '商标侵权的认定标准', '商标侵权的构成要件和救济方式', '商标侵权行为包括：未经商标注册人许可在同一种或类似商品上使用相同或近似商标、销售侵犯注册商标专用权的商品等。权利人可要求停止侵权、赔偿损失。', '商标,知识产权,侵权', '法智通'),
(8, '刑事拘留的最长期限', '刑事诉讼中拘留期限的法律规定', '公安机关对被拘留的人，认为需要逮捕的，应当在拘留后三日内提请检察院审查批准。特殊情况下可以延长一至四日。对流窜作案、多次作案、结伙作案的重大嫌疑分子可以延长至三十日。', '刑事拘留,刑诉法,期限', '法智通');

-- 管理员账号 (密码: admin123, BCrypt加密)
INSERT INTO `user` (`account`, `password`, `nickname`, `user_type`, `status`) VALUES
('admin', '$2a$10$bHXcLAjYcwXJt3yjXE9DhOddWnRTUuoPbiUpIfhuVM0lN6UcwvAsS', '系统管理员', 'ADMIN', 1);
